...
// На первом экране — живой пример поста, а не цифры и обещания: доказать
// сразу убедительнее, чем рассказать. Персона намеренно ремесленная, мастер
// по мылу: продукт рассчитан не только на коучей и консультантов, и
// формулировки проверяются на этом же примере.
const HERO_EXAMPLE = {
...
// Пять шагов конвейера поданы не как механика («сначала это, потом то»), а
// как то, за что обычно платят отдельно: главная ценность продукта —
// позиционирование и распаковка экспертности, а не только готовый текст в
// конце. Порядок соответствует реальному пайплайну агентов
...
// Заглядывание вперёд, каким продукт станет целиком. Про демо-доступ здесь
// сознательно ни слова — он появляется только у формы ниже.
const ROADMAP = [
...
export function LandingScreen() {
  ...
  // Одно согласованное появление первого экрана при загрузке: заголовок,
      ...
      <section className="landing-section landing-split landing-cta" ref={formSectionRef}>
        <div className="landing-split-head reveal">
          ...
          <div className="landing-demo-note">
            <p>
              <strong>Демо — не витрина, а по-настоящему рабочая система</strong>, только
              упрощённая: короткая анкета вместо полного интервью даёт более общий, но честный
              разбор. Что вы получите:
            </p>
            <ul className="landing-demo-list">
              <li>Портрет аудитории и распаковку вашего метода</li>
              <li>Контент-план на две недели</li>
              <li>Готовый пост с картинкой — на каждую вашу площадку</li>
              <li>Для ВК — две-три идеи рилсов, один развёрнутый сценарий и видео по нему</li>
            </ul>
            <p className="landing-demo-scope">Пока работаем только с Telegram и ВК.</p>
          </div>
        </div>
