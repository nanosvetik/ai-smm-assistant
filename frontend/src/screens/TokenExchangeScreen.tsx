...
export function TokenExchangeScreen() {
  ...
  const messages: Record<Exclude<Status, "loading" | "ready">, { title: string; body: string }> = {
    used: {
      title: "Анкета уже отправлена",
      body: "Ссылка перестала действовать в тот момент, когда вы отправили анкету. Если нужно что-то поправить, откройте её напрямую — на этом устройстве доступ держится сутки, и ответы подставятся.",
    },
    expired: {
      title: "Срок действия ссылки истёк",
      body: "Ответьте на письмо, с которым пришла ссылка, и мы вышлем новую.",
    },
    not_found: {
