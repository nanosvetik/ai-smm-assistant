...
- **`GET /api/access/:token`** — обмен одноразовой ссылки на сессию (httpOnly cookie). Сгорает по факту первого использования, не по IP. `SESSION_TTL_MS` = 24 ч, повторного входа в кабинет нет (осознанно, см. decision-log).
...
- Ссылка (`kind: "results"` в `access_links`, не сгорает при использовании, TTL 90 дней) генерируется **автоматически**, не оператором: `agents/resultsDelivery.ts` (`ensureResultsLinkSent`) вызывается fire-and-forget после каждого сохранения поста/сценария (`routes/agents.ts`), идемпотентно проверяет готовность всего текстового демо под реальные площадки клиента и отправляет письмо.
