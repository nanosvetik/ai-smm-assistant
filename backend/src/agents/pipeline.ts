...
// Прогоняет весь текстовый конвейер разом — от ответов анкеты до готовых
// демо-постов и промптов для картинок. Сама генерация картинок и видео сюда
// не входит: она платная и запускается только явным нажатием. Каждый этап
// по-прежнему сохраняет свой результат самостоятельно, эта функция лишь
// задаёт порядок и то, что можно считать параллельно.
